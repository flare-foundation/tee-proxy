...
func Initialize(ctx context.Context, cfgPath string) {
	...
	*infoService = info.NewService(db, actionQueues, resultStorage, &cfg.InfoTiming)
	...
	externalServer := server.NewExternal(cfg.Ports.External, &instructionService, resultService, infoService, walletService, privKey, cfg.DirectExtension, actionQueues, cfg.DirectAPIKey)

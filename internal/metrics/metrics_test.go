...
func TestRuntimeGroupGatesBaseline(t *testing.T) {
	...
	// The process collector emits process_* only where procfs (/proc) exists — Linux, the
	// deployment target. On macOS/Windows dev machines it registers but collects nothing, so
	// only assert presence on Linux.
